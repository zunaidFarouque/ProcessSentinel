...
## 9. GPU VRAM & Temperature Watcher

### Overview
Monitors NVIDIA GPU utilization, video memory allocation (MB), and core temperature to protect hardware from thermal throttling and catch silent AI/ML training crashes.

### How It Works
Executes the native `nvidia-smi` command-line utility with zero external Python library dependencies. Queries the specified GPU index and alerts when temperature exceeds a safe ceiling or when VRAM drops below a floor.

### Parameters
| Parameter | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| **GPU Index** | Integer | `0` | Zero-based index of the target GPU (e.g. `0` for primary, `1` for secondary). |
| **Metric** | Dropdown | `temperature` | `temperature` (°C), `vram_used_mb`, `vram_free_mb`, or `gpu_util_percent`. |
| **Condition** | Dropdown | `above` | `above` or `below`. |
| **Threshold** | Float | `85.0` | Value threshold (°C for temp, MB for VRAM, % for utilization). |

---

## 10. Active Log File Scanner / Tailer

### Overview
Tails active log files (`*.log`, `*.txt`) generated by long-running processes (e.g., PyTorch, GIS jobs, database dumps) to detect critical errors or job completion markers.

### How It Works
Tracks the exact file byte offset (`last_file_pos`) to read only newly appended lines. Automatically detects log file rotation or truncation and scans lines against custom keywords or regular expressions.

### Parameters
| Parameter | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| **Log File Path** | Text | *(Required)* | Absolute path to the log file. Use the **Browse** button. |
| **Patterns to Watch** | Text | `ERROR; FATAL; Traceback` | Semicolon-separated list of keywords or regex patterns. |
| **Regex Match** | Checkbox | `False` | Treat patterns as Python regular expressions. |
| **Case Sensitive** | Checkbox | `False` | Enforce case-sensitive matching. |

---

## 11. System Power & Battery State Check

### Overview
Monitors Windows system power supply status and remaining battery percentage to safeguard unattended batch runs during power outages.

### How It Works
Uses Windows power sensors via `psutil`. If running on a desktop without a battery, displays an OK status without false positives. If running on a laptop or UPS-backed machine, alerts immediately upon AC mains power loss or low battery percentage.

### Parameters
| Parameter | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| **Alert on Battery (AC Lost)** | Checkbox | `True` | Dispatches an alert when AC power disconnects and system runs on battery. |
| **Low Battery Threshold (%)** | Float | `20.0` | Dispatches an urgent alert when battery falls below this percentage. |

---

## 12. Self-Healing Trigger Actions (All Monitors)

Every monitor supports an optional **Trigger Action Command** located under the *Notification Customization & Styling* accordion:

* **Trigger Action Command**: A Windows command or script path (e.g. `powershell -File .\scripts\clean_temp.ps1`, `net start postgresql`, `shutdown /h`).
* **Execution Timeout (sec)**: Maximum allowed runtime before the command is safely terminated (default: `30` seconds).
* **Execution Feedback**: The command runs in an asynchronous background daemon thread when the alert condition triggers, capturing stdout/stderr and logging the result directly to the Live Activity Log and persistent disk log.
